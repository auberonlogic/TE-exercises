...
SELECT 'Ryan' AS name_of_course;
-- single quotes for Strings
-- AS -> alias, names column
...
SELECT state_name FROM state WHERE census_region IS NOT NULL;
...
-- DISTINCT looks at the whole row being SELECTed, so if it's just city_names, repeats will be omitted
...
SELECT city_name
FROM city
WHERE city_name LIKE 'A%'; -- wildcard is percent sign
; -- ILIKE returns case insensitive results

-- The names of all cities that end with "Falls"
SELECT city_name
FROM city
WHERE city_name LIKE '%Falls'; -- wildcard is percent sign
;

-- The names of all cities that contain a space
SELECT city_name
FROM city
WHERE city_name LIKE '% % %'; -- wrapped in wildcards signifies character is contained in the String
;
