...
public class Exercises {
	...
	public int sum13(int[] nums) {
		...
		// to return sum, nums length must be greater than 0
		if (nums.length > 0) {

			// if current value at index is 13 or the previous value at index was 13, omit the following number to the sum
			// use a boolean?
			// set is13 to true?

			for (int i = 0; i < nums.length; i++) {
				// this will add all values to sum.
				// sum += nums[i];

				// now I need to limit it.
				// if neither the value at the current index nor the value at the previous index is 13, add the value to the sum.
